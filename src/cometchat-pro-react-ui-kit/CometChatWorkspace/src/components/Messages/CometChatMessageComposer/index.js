...
import { CometChatGiphyPicker } from "../CometChatGiphyPicker";
...
import insertGif from "./resources/gif.png"; 
...
class CometChatMessageComposer extends React.PureComponent {
	...
	componentDidUpdate(prevProps, prevState) {
		...
		this.item = this.context.type === CometChat.ACTION_TYPE.TYPE_USER 
		|| this.context.type === CometChat.ACTION_TYPE.TYPE_GROUP ? this.context.item : null;
		this.enableLiveReaction();
		this.enablePolls();
		this.enableTypingIndicator();
		this.enableStickers();
	...
	gifClicked = (gifInfo) => {
		console.log(gifInfo.downsized.url, "msg composer ")
		console.log("gif clicked", this.state.messageInput)

		this.setState({messageInput: gifInfo.downsized.url, messageType: "text"}, () => {
	...
	sendTextMessage = () => {
		...
		if (this.state.gifViewer) {
			this.setState({gifViewer: false});
		}

		console.log(this.state.messageInput, "msgInput1");
		if (!this.state.messageInput.trim().length) {
			return false;
		}
		console.log(this.state.messageInput, "msgInput2");
	...
	toggleGifPicker = () => {
		...
		this.setState({gifViewer: !gifViewer, stickerViewer: false});
	};
	...
	render() {
		...
		const gifText = Translator.translate("GIF", this.context.language);
		let gifBtn = (
			<div
				title={gifText}
				css={gifButtonStyle(insertGif, this.context)}
				className="button__gif"
				onClick={() => {
		...
		let sendBtn = (
			<div title={sendMessageText} css={sendButtonStyle(sendBlue, this.context)} className="button__send" 
		...
		let gifViewer = null;
		if (this.state.gifViewer) {
			gifViewer = <CometChatGiphyPicker gifSelected={this.gifClicked} lang={this.context.language} />;
